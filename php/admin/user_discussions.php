...
switch ($method["choice"]){
    ...
    case "add_discussions" :
        ...
        $query=$db->prepare("INSERT INTO user_discussions(discussions_id,user_id_discussions,creator) VALUES(:discussions_id,:user_id_discussions,1)");
        $query->bindValue(":user_id_discussions",$_SESSION["id"]);
    ...
    case "add_participants" :
        ...
        if(!isset($method["user_id_discussions"],$method["discussions_id"])){
            echo json_encode(["success"=>false, "error"=>"Donnees manquantes"]);
            die;
        }
        if(empty(trim($method["discussions_id"])) || empty(trim($method["user_id_discussions"]))){
            echo json_encode(["success"=>false, "error"=>"Donnees vides"]);
            die;
        }
        $query=$db->prepare("INSERT INTO user_discussions(discussions_id,user_id_discussions,creator) VALUES(:discussions_id,:user_id_discussions,0)");
        $query->bindValue(":user_id_discussions",$method["user_id_discussions"]);
        $query->bindValue(":discussions_id",$method["discussions_id"]);
        $query->execute();
        echo json_encode(["success"=>true]);
        break;
    ...
    case "kick_user_discussions" :

        if(isset($method["user_id_discussions"],$method["discussions_id"]) && !empty(trim($method["user_id_discussions"])) && !empty(trim($method["discussions_id"]))){
            $query=$db->prepare("DELETE FROM user_discussions WHERE user_id_discussions=:user_id_discussions AND discussions_id=:discussions_id AND creator=0");
            $query->bindValue(":user_id_discussions",$method["user_id"]);
            $query->bindValue(":discussions_id",$method["discussions_id"]);
            $query->execute();
