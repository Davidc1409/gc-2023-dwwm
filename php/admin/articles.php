...
switch ($method["choice"]){
    ...
    case "insert_articles" :
        ...
        $upload_img = "";
        $upload_bind_img="";
        if (isset($_FILES["media_content"]["name"])){
            $upload_img = ", media_content";
            $upload_bind_img= ", :img";
        } 
        ...
        $query=$db->prepare("INSERT INTO articles($list_concat_column $upload_img) VALUES($list_concat_placeholder $upload_bind_img)");
        $query->bindValue(":user_id_articles",$_SESSION["id"]);
        foreach($list_str_column as $key => $val){
            if(isset($val) && $key!="user_id_articles"){
                $query->bindValue(":".$key,$method[$key]);
            }
        }

        $img_bind = false;
            if (isset($_FILES["media_content"]["name"])) $img_bind = upload($_FILES);
            if ($img_bind) $query->bindValue(":img", "../ressources/images/" . $img_bind);
    ...
    case "update_articles" :
        ...
        $upload_img = "";
        if (isset($_FILES["media_content"]["name"])) $upload_img = ", media_content = :img";
        ...
        $query=$db->prepare("UPDATE articles SET $list_concat $upload_img WHERE id=:id");
        $query->bindValue(":id",$method["id"]);
